(globalThis as any).Deno = { serve: () => {}, env: { get: () => undefined } };
// =============================================================================
...
async function hashIp(req: Request): Promise<string | null> {
  ...
  if (!ip) return null;
  const salt = Deno.env.get("SIGNATURE_IP_SALT") ?? "apex-service-plans";
...
export { buildAgreement, renderPdf, wrap, safe, canonical };
