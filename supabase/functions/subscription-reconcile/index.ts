...
const FN_VERSION = "2026-09-07.v24.7-reconcile-runtime-fix1";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!workerOk) {
      ...
      if (!ok) {
        await safeRpc(db, "record_sync_forbidden");
        return Response.json({ error: "forbidden" }, { status: 403, headers: CORS });
      }
    }

    if (!TOKEN) {
      await safeRpc(db, "record_sync_health", { p_source: source, p_ok: false, p_error: "helcim_not_configured" });
    ...
    if (error) {
      await safeRpc(db, "record_sync_health", { p_source: source, p_ok: false, p_error: "subscription_query_failed" });
    ...
    await safeRpc(db, "record_sync_health", {
      p_source: source,
      p_ok: true,
      p_error: null,
    ...
    if (source === "admin" && wantsOne) {
      await safeQuery(db.from("service_plan_events").insert({
    ...
    console.error("[subscription-reconcile] unhandled", e);
    return Response.json(
      { error: "internal_error", fn_version: FN_VERSION },
      { status: 500, headers: CORS },
    );
  }
});

// Supabase PostgREST builders are PromiseLike (they expose `.then`) but are not
// native Promises and do not reliably expose `.catch`. Calling `.catch(...)` on
// a builder can therefore throw before the request even executes. Keep optional
// health/audit writes best-effort without ever crashing reconciliation.
async function safeQuery(query: any): Promise<void> {
  try { await query; } catch (e) {
    console.error("[subscription-reconcile] best-effort Supabase write failed", e);
  }
}

async function safeRpc(db: any, fn: string, args?: Row): Promise<void> {
  await safeQuery(args === undefined ? db.rpc(fn) : db.rpc(fn, args));
}
