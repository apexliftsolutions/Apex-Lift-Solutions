...
# V23 ADDENDUM — RETRY AFTER PROVIDER REVERSAL

A new defect was found after v22:

- v22 correctly records a provider reverse as a separate `kind='reversal', status='succeeded'` ledger row.
- v22 correctly recalculates the valid invoice back to `unpaid`.
- v22 correctly shows a new Pay button.
- BUT `payment-checkout` still looped over historical `kind='payment'` rows and returned `already_paid` as soon as it saw the original succeeded payment.
- The customer browser could also retain `PAY_BUSY=true` from the original SUCCESS even after the authoritative reversal unlocked `LOCKED_INVOICES`.

V23 fixes both.

`payment-checkout` now uses net retained accounting for retry eligibility:

```
net retained = succeeded payment cents
             - succeeded refund cents
             - succeeded reversal cents
```

A new checkout is allowed only if:
- invoice.status = unpaid
- net retained = 0
- no pending/unknown/fresh initiated payment attempt exists

Historical succeeded payments are preserved and no longer permanently block a fully reversed invoice.

A defensive rule refuses checkout with `payment_under_review` if invoice.status is unpaid but net retained > 0.

`portal-customer.js` now clears both guards after an authoritative successful reversal:
- `LOCKED_INVOICES.delete(invoiceId)`
- `PAY_BUSY = false` for that current/closed checkout

No SQL is required for V23. Preserve all V22 refund/void accounting and email changes.
