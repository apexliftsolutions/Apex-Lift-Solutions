...
(function setupTracking() {
  // Analytics goes through the consent gate, which is a no-op until the visitor
  // opts in and which strips any parameter that looks like identifying data.
  // Calling gtag directly would fire whenever the tag happened to be present.
  function track(eventName, params) {
    try {
      if (window.ApexConsent && typeof window.ApexConsent.track === 'function') {
        window.ApexConsent.track(eventName, params || {});
      }
    } catch (e) { /* analytics must never interfere with the page */ }
  }
  ...
  document.querySelectorAll('a[href^="mailto:"]').forEach(el => {
    el.addEventListener('click', () => {
      track('email_click', { event_category: 'contact' });
    });
  });
