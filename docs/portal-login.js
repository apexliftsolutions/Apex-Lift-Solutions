...
async function handleLogin() {
  ...
  try {
    ...
    // Only `status` and the row's existence are used below. This read sits on
    // the authentication path, so it fetches nothing else.
    const { data: profile } = await _sb.from('customers').select('id,status').eq('id', data.user.id).single();
    showLoad(false);
